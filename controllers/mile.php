		...
		*/
		...
		for($x = $inchX; $x < ($inchX + $width); $x++){
			for($y = $inchY; $y < ($inchY + $height); $y++){
				...
				$sql = "INSERT INTO mile_mile.$table 
		...
		foreach($check as $c=>$col){
			foreach($col as $r=>$row){
				...
				$sql = "SELECT col, row, historyId FROM mile_mile.$table T 
						LEFT JOIN mile_users.content_history H 
						ON T.historyId = H.id
		...
		if( count($delTables) ){
			...
			foreach($delTables as $k=>$v){
				$delSql = "DELETE FROM mile_mile.$v 
