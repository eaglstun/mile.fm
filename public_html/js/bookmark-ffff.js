(function () {
	var token = 'Ue6D7/FZPsqYCSlkZuaQujM9fPemVMoRzErdm9onOO4gTuo0rA/XyOIXxZFpI9lMXrkfmjP7xnAPtIwaYNcVDPo80+osY8BMCShFyXjwWUU=';
	...
	for (var i = 0; i < imgs.length; i++) {
		...
		if (img.src.match(regex_found)) {
			...
			img.onmouseover = function () {
				...
				ele_popup.style.left = offset[0] + ((this.width + (style_em_size * 2)) / 2) - (res_popupimg_w / 2) + 'px'; ele_popup.style.top = offset[1] + ((this.height + (style_em_size * 2)) / 2) - (res_popupimg_h / 2) + 'px';
	...
	function addToFound() {
		...
		if (true) {
			window.open(urlb.join(''), 'found' + new Date().getTime(), 'status=no,resizable=no,scrollbars=no,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,' + 'width=300,height=50,left=0,top=0');
		};
